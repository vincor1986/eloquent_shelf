...
import { DiscoverFormData, InitialBookResults } from "@/types/ai";

export const fetchBookRecommendations = async (formData: DiscoverFormData, existing: string[]): Promise<{ success: boolean; books?: any[]; error?: any }> => {
  const { forMe, forName, forInterests } = formData;

  const prompt = generatePrompt(formData, existing);

  try {
    const response: InitialBookResults = await gptRequest(prompt);
    const { books } = response;
    
    if (!books || books.length === 0) {
      return { success: false, error: "No book recommendations found" };
    }
    ...
    for (let index = 0; index < books.length; index++) {
      ...
      books[index] = {
        ...
        ratings_count,
        page_count,
        isbn: (authorCheckScore || 0) > 0.65 ? isbn : null,
        blurb: blurbs[index],
